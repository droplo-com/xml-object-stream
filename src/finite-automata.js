...
function run(type, args) {
	...
	for (const cb in this._state) {
		if (__own.call(this._state, cb)) {
			if (__own.call(cbs, cb)) {
				const length = cbs[cb].length;
				const cbList = cbs[cb];
...
FiniteAutomata.prototype.on = function (type, state, cb) {
	...
	const typeCbs = this._callbacks[type];
...
FiniteAutomata.prototype.nextState = function (symbol) {
	const newState = {};
	for (const st in this._state) {
		if (__own.call(this._state, st)) {
			if (__own.call(this._states, st)) {
				const next = this._states[st];
...
FiniteAutomata.prototype.transition = function (stateFrom, symbol, stateTo) {
	...
	let exists = __own.call(s, symbol);
	if (exists) {
		s = s[symbol];
	} else {
		s = s[symbol] = {};
