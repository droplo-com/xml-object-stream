...
const __own = Object.prototype.hasOwnProperty;
...
XmlStream.prototype.on = function (eventName, listener) {
	...
	if (event !== null) {
		...
		const finalState = getFinalState.call(this, event.selector);
		const self = this;
		if (event.type === "updateElement") {
			...
			const fn = function (element, context, trace) {
				self.emit(event.name, element, context, trace);
			};
...
XmlStream.prototype.collect = function (selector) {
	...
	const finalState = getFinalState.call(this, selector);
	const self = this;
...
function parseEvent(event) {
	...
	const selector = normalizeSelector(eventParts[2]);
	return {
		selector: selector,
		type: eventType,
		name: (eventParts[2]) ? eventType + ": " + selector.normalized : eventType
	};
}
...
function parse() {
	const self = this;
	const xml = new expat.Parser("utf-8");
	this._parser = xml;
	this._suspended = false;
	const stack = [];
	const trace = {};
	...
	xml.on("endElement", function (name) {
		...
		if (curr.collect) {
			const container = prev.element[name];
	...
	xml.on("text", function (text) {
		...
		if (!self._preserveAll) {
			...
			let spaced = curr.element.$text.substr(0, 1);
			spaced = (spaced !== "") && (spaced.trim() === "");
			let after = curr.element.$text.substr(-1, 1);
